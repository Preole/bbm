...
var BBMCompiler = (function()
{
 ...
 DEF_OPT_HTML =
 {
  ...
  //If > 0, all <h1> - <h6> will be incremented by 1, 
  //up to a maximum of 6.
  MIN_HEADER : 0,
  XHTML : 0
 },

 OPT_BASE = DEF_OPT_BASE,
 OPT_HTML = DEF_OPT_HTML,//HTML Specific: Only applies to (X)HTML output.
 ...
 "\\u2423\\v\\f\\r\\n\\u0085\\u2028\\u2029]+",
 REGEX_CTRL_STR = "[\\u0000-\\u001f\\u007f-\\u009f\\u2028\\u2029]+",
 REGEX_WS_HEAD_STR = "^" + REGEX_WSNL_STR,
 REGEX_WS_TAIL_STR = REGEX_WSNL_STR + "$",
 ...
 REGEX_WS_HEAD = new RegExp(REGEX_WS_HEAD_STR),
 REGEX_WS_TAIL = new RegExp(REGEX_WS_TAIL_STR),
 REGEX_CTRL_G = new RegExp(REGEX_CTRL_STR, "g"),
 REGEX_WS_ONLY = new RegExp("^$|^" + REGEX_WSNL_STR + "$"),
 ...
 REGEX_GRAVE = /`/g,
 REGEX_SLASH = /\//g,
 REGEX_JS_URI = /^javascript:/i,
 REGEX_DATA_URI = /^data:/i,
 REGEX_DIGIT = /[0-9]+/,
 ...
 { //Maps opening text-formatting tokens to their respective ending tokens.
 ...
 //Lists of tokens that contain left, or right square brackets.
 LEX_BRACKET_L_MAP = {BRACKET_L : 1, LINK_INT : 1, LINK_CONT : 1},
 LEX_BRACKET_R_MAP = {BRACKET_R : 1},

 //List of link node types, excluding image.
 AST_LINK_MAP = {LINK_IMG : 1, LINK_EXT : 1, LINK_INT : 1, LINK_WIKI : 1},

 //Inline level nodes. Must contain at least one valid node.
 AST_SPAN_MAP =
 {
  SUB : 1,
  SUP : 1,
 ...
 //These block nodes must contain at least one valid node.
 ...
 /* ////////////////////////////////
 Various string processor functions.
 //////////////////////////////// */

 function str_printCSSClass(classArray, sanitize)
 {
  var res = [];
  for (var i = 0, ii = classArray.length; i < ii; i += 1)
  {
   res.push(str_printCSSID(classArray[i], sanitize));
  }
 ...
 {
  return OPT_HTML.CSS_PRE + (sanitize ? str_encodeCSS(str) : str);
 }
 ...
 {
  return str_trimTail(str_trimHead(str));
 }

 function str_trimTail(str)
 {
  return str.replace(REGEX_WS_TAIL, "");
 }

 function str_trimHead(str)
 {
  return str.replace(REGEX_WS_HEAD, "");
 }
 ...
 {
  return str.replace(REGEX_JS_URI, "javascript;")
  .replace(REGEX_DATA_URI, "data;");
 }
 ...
 {//As ECMAScript 5 Array.prototype.lastIndexOf()
 ...
 {//As ECMAScript 5 Array.prototype.filter()
  var res = [];
  for (var i = 0, ii = arr.length >>> 0; i < ii; i += 1)
  {
   var val = arr[i];
   if (func.call(thisPtr, val, i, arr))
   {
    res.push(val);
   }
  }
 ...
 {//Merges obj2's properties into obj1's
 ...
 */
 ...
 function Lexer(BBMCode)
 {
  ...
  function lexExclaim()
  {
   ...
   else if (LA(1) === "<" && OPT_BASE.ALLOW_IMG)
   {
    shiftChar(2);
  ...
  function lexHash()
  {
   ...
   else if (LA(1) === "<" && OPT_BASE.ALLOW_LINK)
   {
    shiftChar(2);
  ...
  function lexPeriod()
  {
   ...
   else if (OPT_BASE.ALLOW_ID && LA_space(1))
   {
    shiftChar(2);
    createToken(LEX_ENUM.LABEL_ID);
   }
   else if (OPT_BASE.ALLOW_CLASS && LA(1) === "." && LA_space(2))
   {
    shiftChar(3);
 ...
 function Parser(tokenList)
 {
  ...
  {
   var blockNode = null;
   var isNotAbuse = nestLvl < OPT_BASE.MAX_BLOCKS;
   skipWSNL();
  ...
  function parsePara(startCol, formatStack, b_stack, noLink)
  {
   ...
   while (!LA_isParaEnd(minCol))
   {
    ...
    { //Text-formatting.
     ...
     var isAbuse = fmtStack.length >= OPT_BASE.MAX_SPANS;
 ...
 function Filter(AST)
 {
  ...
  function filterInline(node)
  {
   ...
   for (i = 0, ii = node.getLength(); i < ii; i += 1)
   {
    ...
    {
     break; //This subtree is valid, containing at least one visible char.
    }
    else if (ch.getLength() > 0 || AST_LINK_MAP[ch.type])
    {
     filterInline(ch); //Dive deeper.
     if (!ch.isDeleted) {break;}
    }
   }
   
   node.isDeleted = (i >= ii && !AST_LINK_MAP[node.type]);
   if (node.isDeleted) {return;} //Base case; Blank text content.
  ...
  function filterTail(node)
  {
   ...
   for (i; i >= 0; i -= 1)
   {
    ...
    break; //Stop on nodes not marked for deletion.
   }
 ...
 //Returns the HTML string from the parse tree.
 function codeGenStr(AST)
 {
  function printTagOpen(node, attrMap)
  {
   ...
   {
    var h_lvl = node.meta.HEADERLVL + OPT_HTML.MIN_HEADER;
   ...
   if (attrMap)
   {
    ...
    {
     attrMap[each] = attrMap[each].substring(0, OPT_HTML.MAX_ATTR_CHARS);
     openTagStr += " " + each + "=\"" +
     str_encodeHTMLAttr(attrMap[each]) + "\"";
    }
   }

   if (AST_XHTML_MAP[node.type] && OPT_HTML.XHTML)
   {
    openTagStr += "/";
   }
   openTagStr += ">";
  ...
  function printLink(node)
  {
   ...
   { //Internal links should target: "#" + CSS_PRE + ID
    url = "#" + str_printCSSID(url);
   }

   if (isImg && REGEX_WS_ONLY.test(altText) && isParentLink)
   { //AltText is empty & Image in link -> URL is the AltText
    altText = url;
   }

   var linkAttr = isImg ? {src : url, alt : altText} : {href : url};
   if (isImg)
   { //If image, return only the open image tag.
    return printTagOpen(node, linkAttr);
   }
   if (node.isType(AST_ENUM.LINK_WIKI))
   { //Give wiki links a class attribute.
    linkAttr["class"] = str_encodeCSS(OPT_HTML.CSS_WIKI);
   }

   if (node.getLength() > 0)
   { //This link has custom display text.
   ...
   printTagClose(node); //Use the URL as link display text otherwise.
  }
  ...
  function printBlock(node, attrMap, lvl, last)
  {
   ...
   for (ci = 0; ci < cc; ci += 1)
   {
    ...
    {
     id = str_printCSSID(ch.meta.ID, true);
     continue;
    }
    ...
    {//Prepare attribute table. Clear id & class buffer.
     if (id || classes)
     {
      ...
      if (classes)
      {
       chAttrTable["class"] = str_printCSSClass(classes, true);
      }
     }
     id = null;
     classes = null;
 ...
 //Returns a DocumentFragment object from the parse tree.
 function codeGenDOM(AST)
 {
  var win = typeof window === "undefined" ? null : window;
  var doc = typeof document === "undefined" ? null : document;
  if (!win || !doc)
  { //No DOM tree if there's no browser backend.
   return null;
  }

  function makeDomNode(node, attrMap)
  {
   var name = AST_HTML_MAP[node.type] || "";
   if (node.isType(AST_ENUM.HEADER) && node.meta.HEADERLVL)
   {
    var h_lvl = node.meta.HEADERLVL + OPT_HTML.MIN_HEADER;
    name = "h";
    name += (h_lvl > 6) ? "6" : (h_lvl + "");
   }
   ...
   if (attrMap)
   {
    ...
    {
     attrMap[each] = attrMap[each].substring(0, OPT_HTML.MAX_ATTR_CHARS);
     domNode.setAttribute(each, attrMap[each]);
    }
   }
   return domNode;
  }

  function makeCodeBlockNode(node)
  {
   var preNode = doc.createElement("pre");
   var codeNode = doc.createElement("code");
   codeNode.appendChild(makeTextNode(node.children[0]));
  ...
  {
   return doc.createTextNode(node.meta.TEXT);
  }

  function makeLinkNode(node)
  {
   var url = node.meta.URL,
    isImg = node.isType(AST_ENUM.LINK_IMG),
    altText = node.meta.ALT, //alt text -> Image node.
   ...
   if (node.getLength() > 0)
   {
    return makeDom(node, linkAttr);
   }
   var domNode = makeDomNode(node, linkAttr);
   if (!isImg)
   {
    domNode.appendChild(doc.createTextNode(node.meta.URL));
   }
   return domNode;
  }

  function makeDom(node, attrMap)
  {
   var cc = node.getLength(),
   ci = 0,
   ch = null,
   ...
   id = null,
   chAttrTable = null,
   domNode = makeDomNode(node, attrMap);
   ...
   for (ci = 0; ci < cc; ci += 1)
   {
    ...
    {//Prepare attribute table. Clear id & class buffer.
     if (id || classes)
     {
      ...
      if (classes)
      {
       chAttrTable["class"] = str_printCSSClass(classes, false);
      }
     }
     id = null;
     classes = null;
    ...
    if (ch.isType(AST_ENUM.CODE_BLOCK))
    {
     domNode.appendChild(makeCodeBlockNode(ch));
    }
    else if (ch.isType(AST_ENUM.TEXT))
    {
     domNode.appendChild(makeTextNode(ch));
    }
    else if (AST_LINK_MAP[ch.type])
    {
     domNode.appendChild(makeLinkNode(ch));
    }
    else
    {
     domNode.appendChild(makeDom(ch, chAttrTable));
    }
    chAttrTable = null;
   }
   return domNode;
 ...
 function setOptions(frontOpt, backOpt)
 {
  OPT_BASE = obj_merge(DEF_OPT_BASE, frontOpt);
  OPT_HTML = obj_merge(DEF_OPT_HTML, backOpt);
 }

 function compileStr(BBMCode)
 {
  return codeGenStr(FrontEnd(BBMCode));
 }
 function compileDOM(BBMCode)
 {
  return codeGenDOM(FrontEnd(BBMCode));
 }
 ...
 return {
  ...
  parser : Parser,
  filter : Filter,
  frontEnd : FrontEnd, //Lexer, Parser, Filter
  compileStr : compileStr, //Compiles to HTML String
  compileDOM : compileDOM, //Compiles to HTML Browser DOM
  setOptions : setOptions, //Option Setter
  AST_ENUM : AST_ENUM, //Types of ASTNode
  LEX_ENUM : LEX_ENUM, //Types of LexToken
